...
def git_clone(repo, directory=None, depth=None, capture_output=False):
...
def git_commit(message, amend=False, capture_output=False):
    command = "git commit --amend" if amend else f"git commit -m \"{message}\""
    return run_command(command, capture_output)

def get_current_commit(capture_output=False):
    """Get the current HEAD commit hash."""
    return run_command("git rev-parse HEAD", capture_output)[1]

def branch_exists(branch_name, capture_output=False):
    """Check if a branch exists locally."""
    return run_command(f"git rev-parse --verify {branch_name}", capture_output)[0] == 0

def git_push(remote_repo_name="origin", branch="main", force=False, delete=False, capture_output=False):
    if delete:
...
def git_pull(remote=None, branch=None, rebase=False, capture_output=False):
...
def git_checkout(branch_name, capture_output=False):
...
def git_diff(target="HEAD", compare_with=None, staged=False, capture_output=False):
...
def git_pull_request(title, body="", base_branch="main", head_branch=None, capture_output=False):
...
def git_custom(custom_cmd, capture_output=False):
    return run_command(f"git {custom_cmd}", capture_output)